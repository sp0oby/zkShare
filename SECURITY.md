...
- **Client-supplied embeddings:** `store` may include `embedding` (exactly **1536** numbers). The server will **not** call an embedding provider for that row’s vector (it still encrypts `value` server-side today). Use the same model/projection policy as search queries for meaningful similarity.
...
3. **Client-side encryption (E2EE store):** Client sends only ciphertext + commitment + optional embedding; server never sees plaintext. Requires new API contract, key distribution, and likely client libraries — not the default `store` body today.
